...
app.get('/', (req, res) => {
  if (req.session.user) {
    ...
    client.send(new ListObjectsCommand({ Bucket: 'patinkasu' }))
            .then(data => {
...
app.get("/download", (req,res)=>{
  if(req.session.user){
    ...
    client.send(new GetObjectCommand({ Bucket: 'patinkasu', Key: name1 }))
    .then(data => {
      res.attachment(name1)
      data.Body.pipe(res)
    })
  }else{
    res.render('hoge.ejs',{name:"ログインがありません"})
  }
})
app.post("/top", (req,res)=>{
  ...
  connection.query('SELECT * FROM user02', (err, results) =>{
    if (err){
      throw err
    }
    if (results){
